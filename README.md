...
- **关键词搜索** — `Cmd+F` 在主视图打开搜索栏，匹配当前可见内容（含绿底新增 / 红底删除），`Cmd+G` / `Shift+Cmd+G` 在匹配间跳转，`Esc` 关闭
- **in-app 编辑模式** — 工具栏「编辑」按钮（或 `Cmd+E`）切换到 textarea 改正文 / frontmatter，`Cmd+S` 原子写盘覆盖原文件；外部修改时弹冲突对话框（覆盖 / 重新读入 / 取消，**不允许静默覆盖**）；App 退出时若 dirty 弹原生保存对话框
- **字号可调** — `Cmd++` / `Cmd+-`，配置持久化
- **窗口记忆** — 大小/位置/字号自动恢复
- **状态栏** — 显示当前文件绝对路径，点击复制
...
5. 右栏勾选「**隐藏对比**」按纯文本浏览当前 commit；勾选「**隐藏注释**」隐藏 `<!-- -->` 笔记；两者独立组合
6. **Cmd++ / Cmd+-** 调整字号
7. **Cmd+F** 搜索关键词；**Cmd+G** / **Shift+Cmd+G** 跳转下一个 / 上一个；**Esc** 关闭搜索
8. 工具栏点「**编辑**」或按 **Cmd+E** 进入编辑模式改正文 / frontmatter；**Cmd+S** 原子写盘；**Esc** 或再按 Cmd+E 退出（dirty 时弹确认）；Cmd+Q 或 Cmd+W 关闭时若 dirty 弹保存对话框
9. 关闭/移动窗口后下次启动自动恢复
...
- 开发 → 测试 → 发布完整流程
- 关键约束（行号渲染、空行显示、跨行选择、编辑模式原子写 + dirty 双轨同步等踩过的坑）
...
frontend/
  ...
  js/
    ...
    views/  × 8                  mainView / commitList / wordcount / toolbar / toast / fontSize / search / editor
